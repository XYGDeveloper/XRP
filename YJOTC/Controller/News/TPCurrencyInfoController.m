...
NSString *const kSelectedKlineTimeIntervalKey = @"kSelectedKlineTimeIntervalKey";
...
- (void)viewDidLoad {
    ...
    if ([kUserDefaults integerForKey:kSelectedKlineTimeIntervalKey] == 0) {
        _interval = 15;
    }else{
        _interval = [kUserDefaults integerForKey:kSelectedKlineTimeIntervalKey];
    }
    ...
    button.backgroundColor = kRiseColor;
    ...
    rbutton.backgroundColor = kFallColor;
...
- (void)toSell:(UIButton *)sender{
    ...
    YTTradeViewController *vc = [YTTradeViewController new];
    vc.currentListModel = self.listModel;
    vc.isTypeOfBuy = (sender.tag == 1);
...
- (void)lasdFootData{
    ...
    param[@"currency"] = self.listModel.currency_id;
...
- (void)loadLatestTradeData {
    ...
    [YTTradeIndexModel requestTradeIndexsWithCurrencyID:self.listModel.currency_id success:^(YTTradeIndexModel * _Nonnull model, YWNetworkResultModel * _Nonnull obj) {
...
-(void)setupUI
{
    ...
    self.title = self.listModel.comcurrencyName;
    _tableView = [[UITableView alloc]initWithFrame:kRectMake(0,kNavigationBarHeight, kScreenW, kScreenH - kNavigationBarHeight- 45) style:UITableViewStyleGrouped];
...
//    [self addRightBarButtonWithFirstImage:kImageFromStr(@"big_icon_tab") action:@selector(fullScreenAction)];
//
//
...
//    view.backgroundColor = kThemeColor;
    ...
    for (NSInteger i = 0; i < arr.count; i++) {
        ...
        YLButton *button = [[YLButton alloc] initWithFrame:kRectMake(w * i, 0, w, 25) title:arr[i] titleColor:k222222Color font:PFRegularFont(14) titleAlignment:0];
        ...
        if (i < 5) {
            [button setTitleColor:kColorFromStr(@"#066B98") forState:UIControlStateSelected];
            
            if (_interval == 15) {
                button.selected = button.tag == 1;
            }else if (_interval == 60){
                button.selected = button.tag == 2;
                
            }else if (_interval == 60*4){
                button.selected = button.tag == 3;
                
            }else if (_interval == 60*24){
                button.selected = button.tag == 4;
            }
            if (button.selected) {
        ...
        if (i == 5) {
            ...
            if (_interval == 1) {
                button.selected = YES;
                [button setTitle:kLocat(@"k_line_1m") forState:UIControlStateNormal];
            }else if (_interval == 5){
                button.selected = YES;
                
                [button setTitle:kLocat(@"k_line_5m") forState:UIControlStateNormal];
            }else if (_interval == 30){
                button.selected = YES;
                
                [button setTitle:kLocat(@"k_line_30m") forState:UIControlStateNormal];
            }else if (_interval == 60*24*7){
                button.selected = YES;
                
                [button setTitle:kLocat(@"k_line_1w") forState:UIControlStateNormal];
            }else if (_interval == 60*24*30){
                button.selected = YES;
                [button setTitle:kLocat(@"k_line_1mon") forState:UIControlStateNormal];
            }
            if (button.selected) {
                [midView addSubview:self.lineView];
                self.lineView.centerX = button.centerX;
    ...
    self.assenblyView.frame = kRectMake(0, 100 + 50 + 25, kScreenW, TotalHeight);
...
//    [self.moreButton setTitle:kLocat(@"k_line_More") forState:UIControlStateNormal];
    ...
    NSMutableDictionary *param = [NSMutableDictionary dictionary];
    param[@"market"] = [self.listModel.comcurrencyName stringByReplacingOccurrencesOfString:@"/" withString:@"_"];
    ...
    [kNetwork_Tool POST_HTTPS:@"/Api/Ajax/kline" andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        ...
        //绘制k线图
        [self.assenblyView drawHistoryCandleWithDataArr:self.dataArray precision:6 stackName:self.listModel.comcurrencyName needDrawQuota:self.currentDrawQuotaName];
...
- (void)favorAction:(UIButton *)button {
    ...
    kShowHud;
    [self.listModel collectWithSuccess:^(BOOL isCollected) {
        kHideHud;
...
-(void)fullScreenAction
{
    ...
    vc.currencyName = [self.listModel.comcurrencyName stringByReplacingOccurrencesOfString:@"/" withString:@"_"];
...
-(void)timeSelectedAction:(UIButton *)button
{
    ...
    if (button.tag < 5) {
        ...
        if (button.tag > 0 && button.tag < 5) {
            [kUserDefaults setInteger:_interval forKey:kSelectedKlineTimeIntervalKey];
        }
...
- (ZXAssemblyView *)assenblyView
{
    if (!_assenblyView) {
        ...
        _assenblyView.backgroundColor = kWhiteColor;
...
-(UIView *)lineView
{
    if (_lineView == nil) {
        ...
        _lineView.backgroundColor = kColorFromStr(@"#066B98");
    }
    return _lineView;
}
-(UIView *)moreView
{
    if (_moreView == nil) {
        ...
        _moreView.backgroundColor = kTableColor;
        ...
        for (NSInteger i = 0; i < arr.count; i++) {
            ...
            YLButton *button = [[YLButton alloc] initWithFrame:kRectMake(w * i, 0, w, _moreView.height) title:arr[i] titleColor:k222222Color font:PFRegularFont(14) titleAlignment:0];
            ...
            [button addBlockForControlEvents:UIControlEventTouchUpInside block:^(UIButton * sender) {
                ...
                [kUserDefaults setInteger:weakSelf.interval forKey:kSelectedKlineTimeIntervalKey];
                [weakSelf.moreButton setTitle:sender.currentTitle forState:UIControlStateNormal];
                weakSelf.moreButton.selected = YES;
                [UIView animateWithDuration:0.25 animations:^{
...
- (UIView *)indexView
{
    if (_indexView == nil) {
        ...
        _indexView.backgroundColor = kTableColor;
