...
-(void)didClickConfirmButton
{
    ...
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:kDeleteDistrict] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        ...
        if (success) {
            ...
            YWDynamicModel *model = [[YWDynamicModel alloc] init];
            model.district_id = weakSelf.toDeleteModel.district_id;
            
            [[NSNotificationCenter defaultCenter] postNotificationName:@"kUserDidDeleteDistrictKey" object:model];
...
//    __weak typeof(self)weakSelf = self;
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:kFollowAction] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        if (success) {
            ...
            YWDynamicModel *model = [YWDynamicModel new];
            model.is_follow = [NSString stringWithFormat:@"%d",button.isSelected];
            model.member_id = _memberId;
...
//            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(userDidChangeDynamicModelAttention:) name:@"kUserDidChangeDynamicModelAttention" object:nil];
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"kUserDidChangeDynamicModelAttention" object:model];
        }else{
            [self showTips:[responseObj ksObjectForKey:kMessage]];
        }
    }];
