...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(userDidSubmitDistrict) name:@"kUserDidDeleteDistrictKey" object:nil];
...
-(UIView *)setupHeaderView
{
    ...
    UIView *topView = [[UIView alloc]initWithFrame:kRectMake(0, 0, kScreenW, h)];
...
//    topView.backgroundColor = kClearColor;
    ...
    UIImageView *bgView = [[UIImageView alloc]initWithFrame:kRectMake(0, 0, kScreenW, h)];
    ...
    icon.image = [UIImage imageNamed:@"bi_logo"];
...
//    [topView addSubview:changeButton];
    ...
    [changeButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
        ...
        vc.cityID = weakSelf.saveModel.group_city;
        kNavPushSafe(vc);
    }];
...
//    _circleNameLabel.text = [NSString stringWithFormat:@"%@%@",model.city_name,model.group_name];
    ...
    _circleInfoLabel.text = [NSString stringWithFormat:@"%@ %@  %@ %@",kLocat(@"R_Message"),model.note_count,kLocat(@"R_Popularity"),model.popularity_count];
    [_circleIconImgView setImageWithURL:model.group_logo.ks_URL placeholder:kImageFromStr(@"bi_logo")];
    ...
    if (_bgView == nil) {
        ...
        for (NSInteger i = 0; i < 3; i++) {
            ...
            if (i == 0) {
                ...
                if ([[LocalizableLanguageManager userLanguage] isEqualToString:CHINESETradition]) {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.comment.count != 0) {
        ...
        for (NSInteger i = 0; i < commentCount; i++) {
            NSString *commentStr ;
            id parentId = model.comment[i][@"parent_id"];
            if ([parentId isKindOfClass:[NSNull class]] || [parentId integerValue] == 0) {//不是回复的评论
...
//        kHideHud;
        if (success) {
            ...
            if ([dic[@"likes"] integerValue]== 0) {
                ...
                [button setTitle:[NSString stringWithFormat:@"%@",dic[@"likes"]] forState:UIControlStateNormal];
            }
            weakSelf.dataArray[button.tag].is_like = [NSString stringWithFormat:@"%@",dic[@"is_like"]];
//            weakSelf.dataArray[button.tag].likes = dic[@"likes"];
            weakSelf.dataArray[button.tag].likes = [NSString stringWithFormat:@"%@",dic[@"likes"]];
