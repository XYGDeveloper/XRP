...
-(void)createBottomView
{
    ...
    UIView *bgView = [[UIView alloc] initWithFrame:kRectMake(0, kScreenH - 45 , kScreenW, 45)];
    bgView.backgroundColor = kColorFromStr(@"#E4A646");
    ...
    UIImageView *img = [[UIImageView alloc] initWithImage:kImageFromStr(@"mai_icon_otc_xxi")];
    ...
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:kRectMake(65 * kScreenWidthRatio, 0, w, bgView.height) title:kLocat(@"OTC_buyDetail_canceldeal") titleColor:kColorFromStr(@"#9BC1ED") font:PFRegularFont(16) titleAlignment:0];
    ...
    dealButton.backgroundColor = kColorFromStr(@"#6189C5");
...
//        if (indexPath.row < 2) {
        ...
        cell.lineView.backgroundColor = kTabbarColor;
        cell.itemLabel.text = self.titleArray[indexPath.row];
        if (indexPath.row == 0) {
            cell.cpyLabel.textColor = kColorFromStr(@"#6189C5");
        }else{
            cell.cpyLabel.textColor = k666666Color;
        }
...
//            cell.cpyLabel.textColor = kColorFromStr(@"#9BBBEB");
            ...
            [self confirgureCell:cell SubViewsWithButton:YES imageStr:@"fu_icon_fuzhi" infoLabelIsHidden:NO infoStr:[NSString stringWithFormat:@"%@",_tradeInfo[@"add_time"]] copyString:@"25,890.23 CNY" copyLabelIsHidden:YES];
        }else if (indexPath.row == 6){
            
            [self confirgureCell:cell SubViewsWithButton:YES imageStr:@"fu_icon_fuzhi" infoLabelIsHidden:NO infoStr:[NSString stringWithFormat:@"%@",_tradeInfo[@"username"]] copyString:@"25,890.23 CNY" copyLabelIsHidden:YES];
//            [self confirgureCell:cell SubViewsWithButton:NO imageStr:@"user_icon_getin" infoLabelIsHidden:YES infoStr:@"" copyString:_tradeInfo[@"username"] copyLabelIsHidden:NO];
        }
...
-(void)showCancelTipsViewWith:(NSInteger)count
{
    ...
    UIButton *midButton = [[UIButton alloc] initWithFrame:kRectMake(0, 125, midView.width, 16) title:kLocat(@"OTC_buyDetail_notyetpay") titleColor:k999999Color font:PFRegularFont(14) titleAlignment:YES];
...
//    NSLog(@"%zd",_limitTime);
    ...
    if (_limitTime > 0) {
        ...
        if ([_headView.leftTimeLabel.text containsString:@"剩"]) {
            _headView.leftTimeLabel.text = [_headView.leftTimeLabel.text stringByReplacingOccurrencesOfString:@"剩" withString:[NSString stringWithFormat:@"%@ ",kLocat(@"OTC_left")]];
        }
        if ([_headView.leftTimeLabel.text containsString:@"分"]) {
            _headView.leftTimeLabel.text = [_headView.leftTimeLabel.text stringByReplacingOccurrencesOfString:@"分" withString:[NSString stringWithFormat:@" %@ ",kLocat(@"OTC_minute")]];
        }
        
        
        if ([_headView.leftTimeLabel.text containsString:@"秒"]) {
            _headView.leftTimeLabel.text = [_headView.leftTimeLabel.text stringByReplacingOccurrencesOfString:@"秒" withString:[NSString stringWithFormat:@" %@",kLocat(@"OTC_second")]];
        }
