...
- (void)loaddats{
    ...
    kShowHud;
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:@"/Bank/banklist"] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        kHideHud;
        NSLog(@"%@",responseObj);
        if (success) {
...
-(void)setupUI
{
    ...
    if (_type != TPOTCPayWayBankControllerTypeList) {
        ...
        NSString *tipsStr = kLocat(@"s0117_OTCBankCardTips");
        CGSize size = [Utilities calculateWidthAndHeightWithWidth:kScreenW - 24 height:1000 text:tipsStr font:PFRegularFont(12)].size;
  
        UIView *footView = [[UIView alloc] initWithFrame:kRectMake(0, 0, kScreenW, size.height + 15 + 10)];
        footView.backgroundColor = kTableColor;
        _tableView.tableFooterView = footView;
        
        UILabel *tipsLabel = [[UILabel alloc] initWithFrame:kRectMake(12, 15, size.width, size.height) text:tipsStr font:PFRegularFont(12) textColor:kColorFromStr(@"E1545A") textAlignment:0 adjustsFont:NO];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.tf.attributedPlaceholder = [[NSAttributedString alloc] initWithString:self.placeHoldArray[indexPath.section] attributes:@{NSForegroundColorAttributeName: k999999Color}];
    cell.tf.clearButtonMode = UITextFieldViewModeWhileEditing;
    
//    cell.tf.userInteractionEnabled = indexPath.section;
    if (indexPath.section == 0) {
        cell.tf.text = kUserInfo.userName;
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:kRectMake(12, 0, 200, headView.height) text:self.titleArray[section] font:PFRegularFont(12) textColor:k222222Color textAlignment:0 adjustsFont:YES];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 1 && _type != TPOTCPayWayBankControllerTypeList) {
        [self hideKeyBoard];
...
-(void)showTipsView
{
    ...
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:kRectMake(0, 150, midView.width/2, 45) title:kLocat(@"k_popview_select_paywechat_edit_cancel") titleColor:kColorFromStr(@"#9CC1EE") font:PFRegularFont(16) titleAlignment:YES];
    ...
    kViewBorderRadius(pwdTF, 0, 0.5,k999999Color);
    ...
    pwdTF.attributedPlaceholder = [[NSAttributedString alloc] initWithString:kLocat(@"k_popview_select_paywechat_pwd_placehoder") attributes:@{NSForegroundColorAttributeName: k999999Color}];
    _deleteTF = pwdTF;
