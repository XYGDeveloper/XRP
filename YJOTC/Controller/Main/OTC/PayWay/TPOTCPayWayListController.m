...
//    [self addLeftBarButtonWithImage:kImageFromStr(@"backIcon") action:@selector(toback)];
...
-(void)setupUI
{
    self.title = kLocat(@"OTC_main_payway");
...
-(void)loadData
{
    ...
    kShowHud;
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:@"/Bank/index"] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        kHideHud;
...
//                [self showTips:kLocat(@"k_popview_list_counter_emptyData")];
                [[EmptyManager sharedManager]showEmptyOnView:self.view withImage:[UIImage imageNamed:@"lay_img_zwjl"] explain:kLocat(@"k_popview_list_counter_emptyData") operationText:kLocat(@"OTC_empty_tips") operationBlock:^{
...
-(UIView *)addView
{
    if (_addView == nil) {
        ...
        [cancelButton setImage:kImageFromStr(@"mai_icon_del") forState:UIControlStateNormal];
