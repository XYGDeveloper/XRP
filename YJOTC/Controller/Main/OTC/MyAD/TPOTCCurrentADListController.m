...
-(void)showTipsView:(UIButton *)button
{
    ...
    UIButton *confirmlButton = [[UIButton alloc] initWithFrame:kRectMake(cancelButton.right, 150, midView.width/2, 45) title:kLocat(@"Confirm") titleColor:kWhiteColor font:PFRegularFont(16) titleAlignment:YES];
    ...
    //撤销广告后广告将会直接切换为完成状态，已扣除的数量及手续费将不再退还（广告发布后5小时内撤销将收取1‰手续费，超过5小时不收取手续费）。且撤销后不可逆转，是否确定撤销？
    //OTC_ad_canceltips1

    UILabel *tipsLabel = [[UILabel alloc] initWithFrame:kRectMake(28, titleLabel.bottom-15, midView.width - 56, midView.height - 45 - titleLabel.height) text:kLocat(@"s0112_OTCCancelTips") font:PFRegularFont(14) textColor:kColorFromStr(@"#666666") textAlignment:0 adjustsFont:YES];
