...
-(void)setupUI
{
    ...
    midView.frame = kRectMake(12,  91 * kScreenHeightRatio + kNavigationBarHeight, kScreenW - 24, 256+10);
    ...
    UILabel *cnyLabel = [[UILabel alloc] initWithFrame:kRectMake(12, 28 +kNavigationBarHeight, kScreenW - 24, 20) text:[NSString stringWithFormat:@"%@ CNY",_tradeInfo[@"money"]] font:PFRegularFont(20) textColor:k222222Color textAlignment:1 adjustsFont:YES];
    
    [self.view addSubview:cnyLabel];
    
    UIImageView *icon = [[UIImageView alloc] initWithFrame:kRectMake(114 *kScreenWidthRatio, 57 + kNavigationBarHeight , 19, 19)];
    icon.image = kImageFromStr(@"fu_icon_sj");
    ...
    UILabel * timeLabel = [[UILabel alloc] initWithFrame:kRectMake(statusLabel.right + 11, 0, 120, 13) text:[NSString stringWithFormat:@"%@15%@00%@",kLocat(@"OTC_left"),kLocat(@"OTC_minute"),kLocat(@"OTC_second")] font:PFRegularFont(12) textColor:k999999Color textAlignment:0 adjustsFont:YES];
    [self.view addSubview:timeLabel];
    timeLabel.centerY = icon.centerY;
    
 
    UIButton *bottomButton = [[UIButton alloc] initWithFrame:kRectMake(0, kScreenH - 45  , kScreenW, 45) title:kLocat(@"OTC_buy_hasPaied") titleColor:kColorFromStr(@"#8D92A0") font:PFRegularFont(16) titleAlignment:1];
    ...
    UIButton *confirmBurron = [[UIButton alloc] initWithFrame:kRectMake(0, kScreenH - 75-27-5, 38, 38)];
    [self.view addSubview:confirmBurron];
    [confirmBurron setImage:kImageFromStr(@"fu_icon_gno") forState:UIControlStateNormal];
    [confirmBurron setImage:kImageFromStr(@"fu_icon_gpre") forState:UIControlStateSelected];
    ...
    UILabel *tipsLabel = [[UILabel alloc] initWithFrame:kRectMake(confirmBurron.right + 5, 0, kScreenW - confirmBurron.right - 5 - 12, 40) text:kLocat(@"OTC_buyconfirm_tips") font:PFRegularFont(12) textColor:k666666Color textAlignment:0 adjustsFont:YES];
    [self.view addSubview:tipsLabel];
    tipsLabel.numberOfLines = 0;
    tipsLabel.y = kScreenH - 75  - 27 ;
...
//    NSLog(@"%zd",_limitTime);
    ...
    if (_limitTime > 0) {
        ...
        if ([_leftLabel.text containsString:@"剩"]) {
            _leftLabel.text = [_leftLabel.text stringByReplacingOccurrencesOfString:@"剩" withString:[NSString stringWithFormat:@"%@ ",kLocat(@"OTC_left")]];
        }
        if ([_leftLabel.text containsString:@"分"]) {
            _leftLabel.text = [_leftLabel.text stringByReplacingOccurrencesOfString:@"分" withString:[NSString stringWithFormat:@" %@ ",kLocat(@"OTC_minute")]];
        }
        
        if ([_leftLabel.text containsString:@"秒"]) {
            _leftLabel.text = [_leftLabel.text stringByReplacingOccurrencesOfString:@"秒" withString:[NSString stringWithFormat:@" %@",kLocat(@"OTC_second")]];
        }
...
//        }
        ...
        NSString *tipsStr = kLocat(@"s0115_plzpayin0min");
        
        NSString *time = [_leftLabel.text stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"%@ ",kLocat(@"OTC_left")] withString:@""];
        
        _tipsLabel.text = [tipsStr stringByReplacingOccurrencesOfString:@"000" withString:time];
...
-(void)confirmPayAction:(UIButton *)button
{
    if (button.selected) {
        ...
        [self showTips:kLocat(@"s0112_OTCConfirmTips")];
    }
