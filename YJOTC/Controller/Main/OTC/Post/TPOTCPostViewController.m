...
-(void)setupBottomView
{
    ...
    UIButton *rightButton = [[UIButton alloc] initWithFrame:kRectMake(leftButton.right, 0, bottomView.width/2, bottomView.height) title:kLocat(@"Cancel") titleColor:kColorFromStr(@"#9CC1EE") font:PFRegularFont(15) titleAlignment:0];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_userInfo) {
        ...
        if ([_currencyInfo[@"newprice"] doubleValue] + [_currencyInfo[@"newprice2"] doubleValue] > 0) {
            ...
            cell.limiteInfoLabel.text = [NSString stringWithFormat:@"%@：￥%@,%@：%@",kLocat(@"OTC_post_upperprice"),_currencyInfo[@"newprice2"],kLocat(@"OTC_post_lowerprice"),_currencyInfo[@"c"]];
...
//        if ([_currencyInfo[@"newprice2"] doubleValue] > 0) {
//            cell.priceTF.placeholder = ConvertToString(_currencyInfo[@"newprice2"]);
//        }
    ...
    [cell.limiteLowButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
    ...
    [cell.limiteHighButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:kRectMake(20, 0, 200, 50) text:kLocat(@"OTC_post_postad") font:PFRegularFont(18) textColor:k222222Color textAlignment:0 adjustsFont:YES];
...
-(void)nextAction
{
    ...
    if (_remarkTV.text.length > 120) {
        [self showTips:kLocat(@"OTC_remarks_length")];
        return;
    }
    ...
    param[@"key"] = kUserInfo.token;
    param[@"token_id"] = @(kUserInfo.uid);
    param[@"price"] = _priceTF.text;
    param[@"num"] = _volumeTF.text;
    param[@"currency_id"] = _model.currencyID;
    if (_lowButton.isSelected) {
        param[@"min_money"] = _lowTF.text;
    }
    if (_highButton.isSelected) {
        param[@"max_money"] = _highTF.text;
    }
    param[@"order_message"] = _remarkTV.text;
    
    kShowHud;
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:@"/TradeOtc/sell_num_check"] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        if (success) {
            kNavPush(vc);
        }else{
...
{
    [self hideKeyBoard];
    _volumeTF.text = [NSString floatOne:ConvertToString(_userInfo[@"sell_num"]) calculationType:CalculationTypeForMultiply floatTwo:ConvertToString(@(slider.value))];
    
    _volumeTF.text = [NSString stringWithFormat:@"%.6f",_volumeTF.text.doubleValue];
    
    if (_type == TPOTCPostViewControllerTypeSell) {
        _leftLabel.text = [NSString stringWithFormat:@"%@: %@ %@",kLocat(@"OTC_post_postleft"),[NSString floatOne:ConvertToString(_userInfo[@"sell_num"]) calculationType:CalculationTypeForSubtract floatTwo:_volumeTF.text],_model.currencyName];
    }
...
-(void)textFieldTextDidChange:(NSNotification *)noti
{
    ...
    if (tf == _volumeTF) {
        ...
        if (_volumeTF.text.doubleValue > [_userInfo[@"sell_num"] doubleValue]) {
