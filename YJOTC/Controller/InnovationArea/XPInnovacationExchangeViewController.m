//
//  XPInnovacationExchangeViewController.m
//  YJOTC
//
...
@interface XPInnovacationExchangeViewController ()<UITextFieldDelegate,TPWalletSendKeyboardViewDelegate>
...
@property (nonatomic,strong)XPExchangeInnocationModel *model;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *naviHei;

@end

@implementation XPInnovacationExchangeViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = kLocat(@"XPInnovationHomeViewController_exchange");
    self.naviHei.constant = kNavigationBarHeight;
    self.view.backgroundColor = kColorFromStr(@"#F4F4F4");
    [self addRightBarButtonItemWithTitle:kLocat(@"XPInnovacationExchangeViewController_title") titleColor:kColorFromStr(@"#FFFFFF") action:@selector(recorder)];
    ...
    [self.tableview registerNib:[UINib nibWithNibName:@"XPInnocationExcangeTableViewCell" bundle:nil] forCellReuseIdentifier:NSStringFromClass([XPInnocationExcangeTableViewCell class])];
...
- (void)loadData{
    ...
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:@"Exchange/exchange_index"] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        kHideHud;
        NSLog(@"%@",responseObj);
        if (success) {
            XPExchangeInnocationModel *model = [XPExchangeInnocationModel mj_objectWithKeyValues:[responseObj ksObjectForKey:kResult]];
            self.model = model;
...
-(void)didClickConfirmButtonWith:(TPWalletSendKeyboardView *)keyboardView toSubmit:(BOOL)toSubmit
{
    ...
    kShowHud;
    [kNetwork_Tool POST_HTTPS:[Utilities handleAPIWith:@"Exchange/to_exchange"] andParam:param completeBlock:^(BOOL success, NSDictionary *responseObj, NSError *error) {
        kHideHud;
        if (success) {
            kLOG(@"操作成功");
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                kNavPush([XPInnovacationGACListViewController new]);
            });
        }else{
            [kKeyWindow showWarning:[responseObj ksObjectForKey:kMessage]];
        }
...
//        });
        ...
        XPInnocationExcangeTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([XPInnocationExcangeTableViewCell class])];
