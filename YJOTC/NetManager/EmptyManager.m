...
- (EmptyView *)showEmptyOnView:(UIView *)parentView
    ...
    view.backgroundColor = kRGBA(244, 244, 244, 1);
...
- (EmptyView *)showNetErrorOnView:(UIView *)parentView
    ...
    EmptyView *view = [[EmptyView alloc] initWithFrame:CGRectMake(0, 150 *kScreenHeightRatio, parentView.width, parentView.height)];
    [view netErrorLayout];
    [view refreshWithImage:[UIImage imageNamed:@"lay_icon_noint"] explain:kLocat(@"net_alert_load_page") operationText:kLocat(@"net_alert_load_refresh") operationBlock:^{
        [self removeEmptyFromView:parentView];
        if (opBlock) {
            opBlock();
        }
...
- (void)refreshWithImage:(UIImage *)image
    ...
    self.emptyImgView.size = CGSizeMake(60, 60);
...
//        _emptyImgView.contentMode = UIViewContentModeScaleAspectFill;
        _emptyImgView.image = kImageFromStr(@"nodata_icon");
...
//        [_operationButton setBackgroundImage:[UIImage imageWithColor:[UIColor lightGrayColor]] forState:UIControlStateNormal];
        _operationButton.backgroundColor = kColorFromStr(@"#6189C5");
