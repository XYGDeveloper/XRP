...
@property (nonatomic, assign) BOOL isLayoutByProgress;
@property (nonatomic, assign) CGFloat currentPositionX;
@end
...
- (void)layoutLayersAnimated:(BOOL)animated
{
    ...
    CGFloat x = contentFrame.origin.x + stepWidth * self.index;
    if (self.isLayoutByProgress) {
        x = self.progress * CGRectGetWidth(_trackLayer.bounds) + maxRadius;
        x = MIN(x, CGRectGetWidth(_trackLayer.bounds) + maxRadius);
    }
    _sliderCircleLayer.position = CGPointMake(x, CGRectGetMidY(contentFrame));
...
- (CGFloat)progressCalculate {
    CGFloat progress = (self.currentPositionX - maxRadius) / CGRectGetWidth(_trackLayer.bounds);
...
{
    self.isLayoutByProgress = NO;
    startTouchPosition = [touch locationInView:self];
    startSliderPosition = _sliderCircleLayer.position;
    self.currentPositionX = startTouchPosition.x;
    if (CGRectContainsPoint(_sliderCircleLayer.frame, startTouchPosition)) {
        ...
        for (NSUInteger i = 0; i < _trackCirclesArray.count; i++) {
            ...
            if (CGRectContainsPoint(frameToCheck, startTouchPosition)) {
                ...
                //                    [self sendActionsForControlEvents:UIControlEventValueChanged];
                //                }
                CGFloat newProgress  =  (CGFloat)i / ((CGFloat)_trackCirclesArray.count - 1);
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    self.currentPositionX = limitedPosition;
    withoutCAAnimation(^{
        ...
        if (self->_index != index) {
            ...
            self->_index = index;
            //            [self sendActionsForControlEvents:UIControlEventValueChanged];
            CGFloat newProgress  =  (CGFloat)index / ((CGFloat)_trackCirclesArray.count - 1);
            if (newProgress != _progress) {
                _progress = newProgress;
                [self sendActionsForControlEvents:UIControlEventValueChanged];
            }
...
- (void)endTouches
{
    ...
    CGFloat newProgress  = [self progressCalculate];
    if (newIndex != _index) {
        _index = newIndex;
        //        [self sendActionsForControlEvents:UIControlEventValueChanged];
    }
